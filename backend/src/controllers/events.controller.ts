...
import {
    getEventsByUser,
    getNotificationsWithEventDetails,
    getStudyPlanByUser,
    getCurrentWeekStart,
    getLogsByUser,
    insertEvent,
    ...
    upsertWeeklyLog,
    type CourseInput,
} from '../services/dal/scheduler.dal'
...
/**
 * Save (create or replace) the full study plan.
 *
 * Body shape — same as before so the frontend requires no changes:
 * {
 *   name?: string
 *   courses: Array<{
 *     course:      string
 ...
 * Body:
 * {
 *   studyPlanCourseId: string   ← uuid of the study_plan_courses row
 *   scheduledHours:   number[]  ← 7 values, index 0=Mon … 6=Sun
 *   dayStatuses:      (string|null)[]  ← 7 values, same index
 * }
 *
 * Note: the old `course` (string) and `studyPlanId` fields have been replaced
 * by `studyPlanCourseId` (FK uuid). The frontend should send the course row id
 * it receives from GET /study-plan.
 */
export const savePlanLogs = async (req: Request, res: Response) => {
    try {
        const userId = req.user!.id
...
/**
 * Delete a course from the study plan.
 *
 * Body: { studyPlanCourseId: string }
 *
 * Deleting the study_plan_courses row cascade-deletes:
 *   - All study_plan_schedule rows for that course (via FK)
 *   - All study_plan_log_days rows for that course (via FK)
 * No manual log cleanup or JSONB array filtering is needed anymore.
 */
export const deleteCourseData = async (req: Request, res: Response) => {
    try {
